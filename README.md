...
python evaluate.py
Results are saved to ../results/raw_results.json.
4. View Report
...
These issues are documented in cerai-tool/CERAI_ISSUES.md. Rather than spending the remaining time modifying the tool's source code, I created a lightweight evaluation script (test-suite/evaluate.py) that implements the same test design principles and produces equivalent structured results. This approach ensures the evaluation is reproducible and the findings are rigorous.
AI Use Disclosure
I used AI assistants as a research, scaffolding, and drafting tool, not as a replacement for technical or domain judgment.
Research: AI helped rapidly parse the CeRAI tool documentation and Docker setup requirements, saving time on repository exploration.
Code scaffolding: AI generated boilerplate for the FastAPI endpoint, Dockerfile, and HTML report template. I then modified these to fit the Indian agriculture domain (e.g., adding ICAR-recommended varieties, banned pesticide lists, Indic language test cases).
...
Contact
Hemant Salunkhe — hemantrs105@gmail.com
